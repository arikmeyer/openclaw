...
## Service-owned runtime integrations

For the engineering-host split, OpenClaw owns the runtime-side contract for staged
service integrations. Those integrations activate from runtime-owned paths such as:

```text
/var/lib/openclaw/integrations/<pack>/current
```

Top-level runtime config/state stays OpenClaw-owned:

- `/etc/openclaw/...`
- `/var/lib/openclaw/...`
- `openclaw-*` services

OpenClaw must not read service-owned skills directly from a service checkout. See
[Runtime integrations](docs/runtime-integrations.md) for the staged-pack manifest,
validation flow, and active-pack inspection commands.
