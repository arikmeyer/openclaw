...
vi.mock("../runtime-integrations.js", async () => {
  ...
  return {
    ...actual,
    resolveActiveRuntimeIntegrationSkillDirs: resolveActiveRuntimeIntegrationSkillDirsMock,
  };
});
...
afterEach(async () => {
  resolveActiveRuntimeIntegrationSkillDirsMock.mockReset();
  resolveActiveRuntimeIntegrationSkillDirsMock.mockReturnValue([]);
  await Promise.all(
...
describe("loadWorkspaceSkillEntries", () => {
  it("handles an empty managed skills dir without throwing", async () => {
    ...
    expect(entries.filter((entry) => entry.skill.source === "openclaw-managed")).toEqual([]);
  });
  ...
  it("includes skills from active staged runtime integrations", async () => {
    const workspaceDir = await createTempWorkspaceDir();
    const runtimeRoot = await createTempWorkspaceDir();
    const runtimeSkillDir = path.join(runtimeRoot, "skills", "tabiplanner");
    await writeSkill({
      dir: runtimeSkillDir,
      name: "tabiplanner",
      description: "TripPlanner integration skill",
    });
    resolveActiveRuntimeIntegrationSkillDirsMock.mockReturnValue([
      {
        packId: "tripplanner",
    ...
    expect(entries.map((entry) => entry.skill.name)).toContain("tabiplanner");
    expect(entries.find((entry) => entry.skill.name === "tabiplanner")?.skill.filePath).toContain(
      path.join(runtimeSkillDir, "SKILL.md"),
    );
  });

  it("keeps runtime integration skills at lower precedence than workspace skills", async () => {
    const workspaceDir = await createTempWorkspaceDir();
    const runtimeRoot = await createTempWorkspaceDir();
    const runtimeSkillDir = path.join(runtimeRoot, "skills", "tabiplanner");
    const workspaceSkillDir = path.join(workspaceDir, "skills", "tabiplanner");
    await writeSkill({
      dir: runtimeSkillDir,
      name: "tabiplanner",
      description: "Runtime integration version",
    });
    await writeSkill({
