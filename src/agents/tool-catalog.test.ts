...
describe("tool-catalog", () => {
  it("includes code_execution, web_search, x_search, web_fetch, display, and update_plan in the coding profile policy", () => {
    ...
    expect(policy!.allow).toContain("display");
  ...
  it("includes display and bundle MCP tools in coding and messaging profile policies", () => {
    expect(resolveCoreToolProfilePolicy("coding")?.allow).toContain("display");
    expect(resolveCoreToolProfilePolicy("messaging")?.allow).toContain("display");
