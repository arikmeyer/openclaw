...
describe("ensureSkillsWatcher", () => {
  ...
  it("watches active staged runtime integration skill roots", async () => {
    const runtimeSkillDir = "/var/lib/openclaw/integrations/tripplanner/current/skills/tabiplanner";
    resolveActiveRuntimeIntegrationSkillDirsMock.mockReturnValue([
      {
        packId: "tripplanner",
    ...
    expect(targets).toEqual(
      expect.arrayContaining([
        posix(path.join(runtimeSkillDir, "SKILL.md")),
        posix(path.join(runtimeSkillDir, "*", "SKILL.md")),
      ]),
    );
  });
