...
import { resolveActiveRuntimeIntegrationSkillDirs } from "../../runtime-integrations.js";
...
function loadSkillEntries(
  ...
  const runtimeIntegrationSkillDirs = resolveActiveRuntimeIntegrationSkillDirs();
  ...
  const mergedExtraDirs = [
    ...
    ...runtimeIntegrationSkillDirs.map((entry) => ({
      dir: entry.dir,
      source: entry.source,
    })),
    ...pluginSkillDirs.map((dir) => ({
      dir: dir,
      source: "openclaw-extra",
    })),
  ];
  ...
  const bundledSkills = bundledSkillsDir
    ...
    : [];
  const extraSkills = mergedExtraDirs.flatMap(({ dir, source }) => {
    return loadSkills({
      dir,
      source,
    });
  ...
  // Precedence: extra/runtime-integration/plugin < bundled < managed < agents-skills-personal < agents-skills-project < workspace
